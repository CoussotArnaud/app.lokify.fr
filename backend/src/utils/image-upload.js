...
export const formatImageUploadSizeLimit = (bytes) =>
  `${Math.round(Number(bytes || 0) / (1024 * 1024))} Mo`;

export const buildImageTooLargeMessage = (maxSizeBytes = MAX_IMAGE_UPLOAD_SIZE_BYTES) =>
  `Le fichier est trop volumineux. Taille maximale autorisee : ${formatImageUploadSizeLimit(
    maxSizeBytes
  )}.`;
...
export const parseImageUploadPayload = (
  payload = {},
...
export const validateImageUploadAsset = (
  asset = {},
  {
    allowedMimeTypes = DEFAULT_ALLOWED_IMAGE_MIME_TYPES,
    ...
    minWidth = 0,
    minHeight = 0,
    tooLargeMessage = buildImageTooLargeMessage(maxSizeBytes),
  } = {}
) => {
  ...
  if (minWidth > 0 && minHeight > 0 && (dimensions.width < minWidth || dimensions.height < minHeight)) {
    throw new HttpError(
      400,
...
export const validateImageUploadPayload = (payload = {}, options = {}) => {
  const parsedImage = parseImageUploadPayload(payload, options);
  const validatedImage = validateImageUploadAsset(parsedImage, options);
