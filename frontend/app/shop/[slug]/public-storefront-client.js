...
export default function PublicStorefrontClient({
  slug: initialSlug = "",
  initialBookingForm = null,
  initialStorefrontData = null,
  initialStorefrontError = "",
}) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const slug = String(initialSlug || "");
  const checkoutStatus = searchParams.get("checkout");
  const checkoutSessionId = searchParams.get("session_id") || "";
  const searchParamsString = searchParams.toString();
  const draftStorageKey = getStorefrontDraftStorageKey(slug);
  const fallbackBookingForm = initialBookingForm || buildDefaultBookingForm();
  const [shopState, setShopState] = useState(() => ({
    loading: !initialStorefrontData && !initialStorefrontError,
    error: initialStorefrontError || "",
    data: initialStorefrontData,
  }));
  const [bookingForm, setBookingForm] = useState(fallbackBookingForm);
  ...
  useEffect(() => {
    ...
    const requestKey = `${slug}::${bookingForm.start_date}::${bookingForm.end_date}`;
    const canReuseInitialResponse =
      reloadVersion === 0 &&
      requestKey === initialRequestKeyRef.current &&
      (initialStorefrontData || initialStorefrontError);

    if (canReuseInitialResponse) {
      return;
    }
